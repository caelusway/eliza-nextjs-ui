...
export function SocialMentions() {
  const { tweets, loading, error, lastFetched } = useTweets({
  ...
  const fallbackTweets = [
    { id: '1', text: '"Stop waiting—fund repair biology."', username: '@samknowsscience', created_at: null, metrics: null },
    { id: '2', text: '"Science moves forward when we fund bold research."', username: '@samknowsscience', created_at: null, metrics: null },
    { id: '3', text: '"The future of longevity research is in our hands."', username: '@samknowsscience', created_at: null, metrics: null },
    { id: '4', text: '"Every dollar invested in aging research saves lives."', username: '@samknowsscience', created_at: null, metrics: null },
    { id: '5', text: '"We need to accelerate biomedical research now."', username: '@samknowsscience', created_at: null, metrics: null },
  ];

  const displayTweets = error || loading || tweets.length === 0 
    ? fallbackTweets.map(tweet => ({
        ...tweet,
        url: `https://x.com/SamKnowsScience` // Fallback to profile
      }))
    : tweets.map(tweet => {
  ...
  if (tweets.length > 0) {
    console.log(`[SocialMentions] Processing ${tweets.length} tweets:`, tweets.map(t => ({
      id: t.id,
