...
export default function CompactInviteFriendsDialog({ isOpen, onClose }: CompactInviteFriendsDialogProps) {
  const { user } = usePrivy();
      ...
      case 'accepted':
        return (
          <div className="flex items-center gap-1 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            <span className="ml-2">{uses}/{maxUses} uses</span>
          </div>
        );
      case 'email_sent':
        return (
          <div className="flex items-center gap-1 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            <span className="ml-2">{uses}/{maxUses} uses • Awaiting signup</span>
          </div>
        );
      case 'pending':
        return (
  ...
  const usedCodesCount = inviteCodes.filter(code => code.status === 'accepted' || code.used).length;
  const availableCodesCount = inviteCodes.filter(code => code.status === 'pending' || (!code.used && code.status !== 'accepted')).length;
  ...
  return (
    <>
            ...
            <div className="px-8 py-6">
              <div className="flex items-center justify-between p-4 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-700/50">
                <div className="flex gap-8">
                  ...
                  <div>
                    <div className="text-lg font-semibold text-zinc-900 dark:text-white">{usedCodesCount}</div>
              ...
              <div className="space-y-3 max-h-[320px] overflow-y-auto overflow-x-visible">
                ...
                {isLoading ? (
                  ...
                  <div className="text-center py-8 text-zinc-600 dark:text-zinc-400">
                    ...
                    <p className="text-xs">Your invite codes will appear here once they are assigned to you</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3 flex-1">
                            <span className="font-mono text-sm text-zinc-900 dark:text-white font-medium">{invite.code}</span>
      ...
      {/* Email Dialog */}
      {emailDialogInvite && (
        <DialogPrimitive.Root open={!!emailDialogInvite} onOpenChange={() => setEmailDialogInvite(null)}>
          <DialogPrimitive.Portal>
            <DialogPrimitive.Overlay className="fixed inset-0 z-50 bg-black/50 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />
            <DialogPrimitive.Content 
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-sm text-zinc-600 dark:text-zinc-400">Your Name (Optional)</label>
                  <input
                    type="text"
                    value={senderName}
